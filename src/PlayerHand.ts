...
import { TOOL_DEFS } from './ToolTextures';
...
export class PlayerHand {
    ...
    constructor(camera: THREE.Camera, blockTexture: THREE.DataTexture, toolTextures: Record<number, any>) {
        this.camera = camera;
    ...
    private createToolMesh(def: any): THREE.Mesh {
        ...
        const addFace = (x: number, y: number, z: number, w: number, h: number, d: number, nx: number, ny: number, nz: number, r: number, g: number, b: number) => {
            ...
            const z0 = z; const z1 = z + d;
            
            // Define 4 corners relative to normal
            let p0, p1, p2, p3;
            
            if (nx === 1) { // Right
                p0 = [x1, y0, z1]; p1 = [x1, y0, z0]; p2 = [x1, y1, z1]; p3 = [x1, y1, z0];
            } else if (nx === -1) { // Left
                p0 = [x0, y0, z0]; p1 = [x0, y0, z1]; p2 = [x0, y1, z0]; p3 = [x0, y1, z1];
            } else if (ny === 1) { // Top
                p0 = [x0, y1, z1]; p1 = [x1, y1, z1]; p2 = [x0, y1, z0]; p3 = [x1, y1, z0];
            } else if (ny === -1) { // Bottom
                p0 = [x0, y0, z0]; p1 = [x1, y0, z0]; p2 = [x0, y0, z1]; p3 = [x1, y0, z1];
            } else if (nz === 1) { // Front
                p0 = [x0, y0, z1]; p1 = [x1, y0, z1]; p2 = [x0, y1, z1]; p3 = [x1, y1, z1];
            } else { // Back
                p0 = [x1, y0, z0]; p1 = [x0, y0, z0]; p2 = [x1, y1, z0]; p3 = [x0, y1, z0];
            }
        ...
        const rgbMat = { r: rgbMatRes.r/255, g: rgbMatRes.g/255, b: rgbMatRes.b/255 };
        ...
        for (let y = 0; y < size; y++) {
            ...
            for (let x = 0; x < size; x++) {
                ...
                let r=1, g=1, b=1;
                if (char === '1') {
                    r = rgbHandle.r; g = rgbHandle.g; b = rgbHandle.b;
                } else if (char === '2') {
                    r = rgbMat.r; g = rgbMat.g; b = rgbMat.b;
                }
    ...
    public update(delta: number, isMoving: boolean) {
        ...
        if (isMoving) {
            ...
            this.handGroup.position.x = this.BASE_POS.x + Math.sin(this.bobTime) * 0.05;
            this.handGroup.position.y = this.BASE_POS.y + Math.abs(Math.cos(this.bobTime)) * 0.05;
        } else {
            // Return to rest
            this.handGroup.position.x += (this.BASE_POS.x - this.handGroup.position.x) * 10 * delta;
            this.handGroup.position.y += (this.BASE_POS.y - this.handGroup.position.y) * 10 * delta;
        }
